...
public class Boss extends Ennemis {
    ...
    public Boss(String name, int maxPv, int atk, double atkSpeed, int range, Position position, Element element, double speed, int reward, Color color, double deltaTime) {
    ...
    public void attaquer(Unite cible) {
        if (cible instanceof Tour) {
            ...
            if (!toursDansPortee.isEmpty()) {
                // Trouver la tour la plus proche dans la portée
